...
def rapihkan_teks(label: str, teks: str, width: int = 35) -> str:
  ...
  return textwrap.fill(
      teks, width=width, initial_indent=label, subsequent_indent=indent_spasi
  )
...
async def main_async():
  print(
      f"DEBUG: Analisa {SYMBOL_SPOT} (spot) | konteks futures: {SYMBOL_SWAP}"
  )
  bot = Bot(token=TOKEN)
  ...
  try:
    ...
    df_1d = pd.DataFrame(
        bars_1d, columns=["timestamp", "open", "high", "low", "close", "volume"]
    ).astype(float)
    ...
    piv_1h, piv_4h, piv_1d = (
        pivot_levels(df_1h),
        pivot_levels(df_4h),
        pivot_levels(df_1d),
    )
    r1_1h_idr, s1_1h_idr = float(piv_1h["r1"] * usd_idr), float(
        piv_1h["s1"] * usd_idr
    )
    r1_4h_idr, r2_4h_idr = float(piv_4h["r1"] * usd_idr), float(
        piv_4h["r2"] * usd_idr
    )
    s1_4h_idr, s2_4h_idr = float(piv_4h["s1"] * usd_idr), float(
        piv_4h["s2"] * usd_idr
    )
    r1_1d_idr, r2_1d_idr = float(piv_1d["r1"] * usd_idr), float(
        piv_1d["r2"] * usd_idr
    )
    s1_1d_idr, s2_1d_idr = float(piv_1d["s1"] * usd_idr), float(
        piv_1d["s2"] * usd_idr
    )
    ...
    ema_up, ema_down = (
        up.ewm(com=13, adjust=False).mean(),
        down.ewm(com=13, adjust=False).mean(),
    )
    rs = ema_up / ema_down
    ...
    vol_spike = bool(
        curr_1h_live["volume"] > (df_1h["avg_vol"].iloc[-1] * 1.8)
    )

    if is_bullish_4h:
      choch = bool(
          curr_1h_live["close"] > curr_1h_live["open"]
          and curr_1h_live["volume"] > (df_1h["avg_vol"].iloc[-1] * 1.5)
      )
      ...
      mitigation = bool(
          (df_4h["high"].iloc[-1] * usd_idr) >= (r1_4h_idr * 0.995)
      )
    ...
    skor_smc, breakdown_skor = hitung_skor_smc(
        choch, bos, mitigation, fvg_active, rrr_4h, vol_spike
    )
    ...
    label_skor = (
        "🔥 SANGAT KUAT"
        if skor_smc >= 70
        else ("✅ POTENSIAL" if skor_smc >= 50 else "⚠️ STANDAR")
    )
